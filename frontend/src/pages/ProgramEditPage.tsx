...
function originalExerciseRef(exercise: DayExerciseData, key: string): OriginalExerciseRef {
  return {
    key,
    exerciseId: exercise.exercise.id,
    supersetGroup: exercise.superset_group,
...
function normalizeSupersetGroups(exercises: DraftExercise[]) {
  ...
  return exercises.map((exercise) => ({
    ...exercise,
    supersetGroup:
      exercise.supersetGroup != null && (counts.get(exercise.supersetGroup) ?? 0) > 1
  ...
  return {
    ...
    weeks: draft.weeks.map((week) => ({
      ...
      days: week.days.map((day) => ({
        title: day.title,
        weekday: day.weekday,
        exercises: normalizeSupersetGroups(day.exercises).map((exercise) => ({
...
function exerciseComparableSignature(
  exercise: { exerciseId: number; sets: DraftSet[]; notes: string },
) {
  return JSON.stringify({
    exerciseId: exercise.exerciseId,
    notes: exercise.notes,
...
function exercisesMatch(
  ...
  return exerciseComparableSignature(left) === exerciseComparableSignature(right);
}
...
function findNextMatch(
  ...
  for (let originalIndex = originalStart; originalIndex < originals.length; originalIndex += 1) {
    for (let currentIndex = currentStart; currentIndex < currents.length; currentIndex += 1) {
      if (exercisesMatch(originals[originalIndex], currents[currentIndex])) {
...
function buildDisplayItems(
  ...
  while (originalIndex < originals.length || currentIndex < currents.length) {
    ...
    const nextMatch = findNextMatch(originals, currents, originalIndex, currentIndex);
...
export function ProgramEditPage({ onClose }: ProgramEditPageProps) {
  ...
  const selectedWeekNumber = useMemo(() => {
    ...
    const index = draft.weeks.findIndex((week) => week.uid === selectedWeekUid);
  ...
  const displayItems = useMemo(
    () => buildDisplayItems(originalDayExercises, selectedDay?.exercises ?? []),
    [originalDayExercises, selectedDay?.exercises],
  );

  const hasChanges = draft ? contentSignature(draft) !== initialContentSignature : false;
  const canSave = draft != null && isDraftValid(draft) && hasChanges && !saving;
  ...
  function requestDeleteExercise(exercise: DraftExercise) {
    ...
    setDeleteTarget({
      ...
      description: "Упражнение исчезнет из итоговой программы, но оригинальные удаления останутся видны в редакторе.",
    });
  }
  ...
  function confirmDeleteExercise() {
    ...
    updateSelectedDay((currentDay) => ({
      ...currentDay,
      exercises: currentDay.exercises.filter((exercise) => exercise.uid !== deleteTarget.exerciseUid),
    }));
    setDeleteTarget(null);
  }

  function restoreRemovedExercise(itemIndex: number, exercise: OriginalExerciseRef) {
    updateSelectedDay((currentDay) => {
      const nextCurrentItem = displayItems
        .slice(itemIndex + 1)
        .find((item): item is Extract<DisplayExerciseItem, { kind: "current" }> => item.kind === "current");
      const insertIndex = nextCurrentItem
        ? currentDay.exercises.findIndex((entry) => entry.uid === nextCurrentItem.exercise.uid)
        : currentDay.exercises.length;
      const exercises = [...currentDay.exercises];
      const targetIndex = insertIndex === -1 ? exercises.length : insertIndex;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <Dialog open={deleteTarget != null} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{deleteTarget?.title ?? "Удалить упражнение?"}</DialogTitle>
